    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:item.actions="{ item }">
          <v-icon class="me-2" size="small" @click="openEditItemDialog(item)">mdi-pencil</v-icon>
          <v-icon size="small" @click="openDeleteItemDialog(item)">mdi-delete</v-icon>
        </template>
      </v-data-table-server>
    </v-card>
...
watch(search, () => {
  // Pequeña espera (debounce) para no enviar peticiones en cada tecla.
  setTimeout(() => {
    loadItems(tableOptions.value);
  }, 500);
});
