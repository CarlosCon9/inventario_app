      ...
      <v-card-text>
        <v-window v-model="tab">
          <v-window-item value="stockBajo">
            <v-btn color="success" @click="exportar('stockBajo')" :loading="loading.stockBajo" class="mb-4">
              <v-icon start>mdi-file-excel</v-icon>
              Exportar a Excel
            </v-btn>
            <v-data-table :headers="headers.stockBajo" :items="data.stockBajo" :loading="loading.stockBajo"></v-data-table>
          </v-window-item>

          <v-window-item value="inventario">
            <v-btn color="success" @click="exportar('inventario')" :loading="loading.inventario" class="mb-4">
              <v-icon start>mdi-file-excel</v-icon>
              Exportar a Excel
            </v-btn>
            <v-data-table :headers="headers.inventario" :items="data.inventario" :loading="loading.inventario">
          ...
          <v-window-item value="movimientos">
            <v-row class="mb-2">
              <v-col cols="12" sm="3"><v-text-field v-model="filters.fechaDesde" label="Desde" type="date"></v-text-field></v-col>
              <v-col cols="12" sm="3"><v-text-field v-model="filters.fechaHasta" label="Hasta" type="date"></v-text-field></v-col>
              <v-col cols="12" sm="2"><v-select v-model="filters.tipoMovimiento" :items="['entrada', 'salida']" label="Movimiento" clearable></v-select></v-col>
              <v-col cols="12" sm="4"><v-autocomplete v-model="filters.parteId" :items="partes" item-title="nombre" item-value="id" label="Filtrar por Parte" clearable @update:search="buscarPartes"></v-autocomplete></v-col>
            </v-row>
            <v-btn color="primary" @click="cargarReporteMovimientos" :loading="loading.movimientos">Generar Reporte</v-btn>
            <v-btn color="success" @click="exportar('movimientos')" :loading="loading.movimientos" class="ml-2">
              <v-icon start>mdi-file-excel</v-icon>
              Exportar
            </v-btn>
            <v-data-table :headers="headers.movimientos" :items="data.movimientos" :loading="loading.movimientos" class="mt-4"></v-data-table>
          </v-window-item>
        </v-window>
      </v-card-text>
    </v-card>
...
const loading = reactive({ stockBajo: false, inventario: false, movimientos: false });
const filters = ref({ fechaDesde: null, fechaHasta: null, tipoMovimiento: null, parteId: null });
const partes = ref([]);
...
const cargarDatosIniciales = async () => {
    ...
    finally { loading.stockBajo = false; loading.inventario = false; }
};
...
const cargarReporteMovimientos = async () => {
    ...
    finally { loading.movimientos = false; }
};
