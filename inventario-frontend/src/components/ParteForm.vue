    ...
    <v-card-text>
      <v-container>
        <v-form ref="form">
          <v-row>
            <v-col cols="12" sm="6">
              <v-text-field v-model="parte.nombre" label="Nombre del Producto *" :rules="[rules.required]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <v-text-field v-model="parte.numero_parte" label="Número de Parte / SKU *" :rules="[rules.required]"></v-text-field>
            </v-col>

            <v-col cols="12">
              <v-textarea v-model="parte.descripcion" label="Descripción" rows="2"></v-textarea>
            </v-col>

            <v-col cols="12" sm="6" md="3">
              <v-text-field v-model.number="parte.cantidad" label="Cantidad en Stock *" type="number" :rules="[rules.required, rules.number]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" md="3">
              <v-text-field v-model.number="parte.cantidad_minima" label="Stock Mínimo" type="number" :rules="[rules.number]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" md="3">
              <v-text-field v-model="parte.unidad_medida" label="Unidad de Medida (ej. Un, Caja, Mt)"></v-text-field>
            </v-col>
             <v-col cols="12" sm="6" md="3">
              <v-text-field v-model="parte.ubicacion" label="Ubicación"></v-text-field>
            </v-col>

            <v-col cols="12" sm="6" md="4">
              <v-text-field v-model.number="parte.precio_compra" label="Precio de Compra" type="number" prefix="$"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" md="4">
              <v-text-field v-model.number="parte.porcentaje_ganancia" label="Porcentaje de Ganancia" type="number" suffix="%"></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="4">
              <v-text-field :model-value="precioVentaCalculado" label="Precio de Venta (Calculado)" type="number" prefix="$" readonly disabled></v-text-field>
            </v-col>

            <v-col cols="12" sm="6">
              <v-text-field v-model="parte.categoria" label="Categoría"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <v-autocomplete
